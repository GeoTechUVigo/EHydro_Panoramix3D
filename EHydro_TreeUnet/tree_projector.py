...
class PixelDecoderAlt(nn.Module):
    def __init__(self, in_channels, channels, latent_dim):
        super().__init__()
        self.upsample = nn.ModuleList(nn.Sequential(
            spnn.Conv3d(sum(channels[i:]), sum(channels[i:]), kernel_size=2, stride=2, transposed=True, bias=False),
...
class TreeProjector(nn.Module):
    def __init__(self, in_channels, num_classes, max_instances, channels = [64, 128, 256], latent_dim = 512):
        super().__init__()
